...
@section('content')
    ...
    <div class="page-content">
        <!-- BEGIN PAGE HEADER-->
        <!-- BEGIN PAGE BAR -->
        <div class="page-bar {{(Auth::user()->isAllow('em_eg_add')) ? "" : "hidden"}}">
                                    ...
                                    @foreach($liste as $key => $row)
                                        <tr>
                                            ...
                                            <td>
                                                ...
                                                @endif
                                                @if(Auth::user()->isAllow('em_eg_del'))
                                            ...
                                            <td>
                                                <div>{{$row->adi}}</div>
                                                <div>
                                                    @if($row->dinamik_grup_id != "")
                                                        <label class="label-success small">Dinamik Group - {{$row->dinamikSorgu->adi}}</label>
                                                        @else
                                                        <label class="label-default">Static Group</label>
                                                    @endif
                                                </div>
                                            </td>
                                            <td>{{$row->statikEmailSayisi->sayisi}}</td>
                                            <td><a href="/em_grouplist/search/{{$row->id}}" target="_blank">Show Contact List</a></td>
                                            <td><a href="/em_grouplist/create" target="_blank">Add Record</a></td>
                                            <td>
                                                <div>{{$row->yetkili1->adi_soyadi}}</div>
                                                <div>{{$row->yetkili2->adi_soyadi}}</div>
                                                <div>{{$row->yetkili3->adi_soyadi}}</div>
                                            </td>
                                        </tr>
                                    @endforeach
                                    </tbody>
