...
@section('content')
    ...
    <div class="page-content">
        <!-- BEGIN PAGE HEADER-->
        <!-- BEGIN PAGE BAR -->
        <div class="page-bar {{(Auth::user()->isAllow('em_mt_add')) ? "" : "hidden"}}">
        ...
        <form method="post" action="/{{$prefix}}/search">
            ...
            <div class="m-heading-1 border-green m-bordered col-md-8">
                <h3>Groups:</h3>
                <p class="col-md-6">
                    <select id="tur_id" name="tur_id" class="form-control select2">
                        <option value="">Select Group/Category</option>
                        @foreach($tur_listesi as $row)
                            <option value="{{$row->id}}"{{$row->id == $tur_id ? " selected" : ""}}>{{$row->adi}}</option>
                        @endforeach
                    </select>
                </p>
                <p class="col-md-2">
                                    ...
                                    @foreach($liste as $key => $row)
                                        <tr>
                                            ...
                                            <td>
                                                ...
                                                @endif
                                                @if(Auth::user()->isAllow('em_mt_del'))
