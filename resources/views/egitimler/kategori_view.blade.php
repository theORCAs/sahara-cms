...
@section('content')
    ...
    <div class="page-content">
        <!-- BEGIN PAGE HEADER-->
        <!-- BEGIN PAGE BAR -->
        <div class="page-bar {{(Auth::user()->isAllow('to_category_add')) ? "" : "hidden"}}">
                                        ...
                                        <tr>
                                            ...
                                            <td>
                                                @if($row->flg_aktif == "0")
                                                    ...
                                                    @else
                                                <div>
                                                    <select id="select_order" data-id="{{$row->id}}" class="form-control" onchange="changeOrder('{{$row->id}}')">
                                                        @for($sira = 1; $sira <= $liste->total(); $sira++)
                                                            <option value="{{$sira}}" {{$sira == $row->sira ? ' selected' : ''}}>{{$sira}}</option>
                                                            @endfor
                                                    </select>
                                                </div>
                                                @endif
...
@section("js")
    ...
    <script type="text/javascript">
        function changeOrder(id) {
            var order = $("#select_order[data-id='" + id + "']").val();
            var data = {
                '_token' : "{{csrf_token()}}",
                'id' : id,
                'order' : order
            ...
            showLoading('', '');
            $.post("{{$prefix}}/changeCategoryOrder", data, function (result) {
                if(result.success) {
                    window.location.reload();
                } else {
                    toastr['error']("{{config('messages.islem_basarisiz')}} " + result.message, '');
                }
            }, "json");
        }
        $('.pagination-panel-input').on('keyup', function(e) {
            if(e.which == 13) {
                var page = $(this).val();
                // window.location.href = "{{$liste->url($liste->lastPage())}}"
            }
        })
    </script>
@endsection
