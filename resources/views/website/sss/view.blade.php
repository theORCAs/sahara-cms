...
@section('content')
    ...
    <div class="page-content">
        <!-- BEGIN PAGE HEADER-->
        <!-- BEGIN PAGE BAR -->
        <div class="page-bar {{(Auth::user()->isAllow('wmo_faq_add')) ? "" : "hidden"}}">
                                        ...
                                        <tr>
                                            ...
                                            <td class="text-center">
                                                @if(Auth::user()->isAllow('wmo_faq_edit'))
                                                    <a href="/{{$prefix}}/{{$row->id}}/edit" class="btn btn-xs btn-primary tooltips" data-container="body" data-placement="right" data-original-title="Update '{{$row->soru}}'"><i class="fa fa-pencil"></i></a>
                                                @endif
                                                @if(Auth::user()->isAllow('wmo_faq_del'))
                                                    <a href="javascript:;" class="btn btn-xs btn-danger tooltips" data-container="body" data-placement="right" data-original-title="Delete '{{$row->soru}}'" onclick="silmeKontrol('{{$row->id}}', '/{{$prefix}}')"><i class="fa fa-trash"></i></a>
                                                @endif
                                            </td>
                                            <td class="text-center">
                                                @if($row->flg_aktif == 1)
                                                ...
                                                @else
                                                    <label class="label label-danger">Passive</label>
                                                @endif
                                            </td>
                                            <td>{{$row->soru}}</td>
                                        </tr>
...
@section("js")
    ...
    <script type="text/javascript">
        ...
        function listesiGetir() {
            window.location.href="/{{$prefix}}/" + $("#ulke_id").val();
        }
    </script>
@endsection
