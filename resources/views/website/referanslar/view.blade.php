...
@section('content')
    ...
    <div class="page-content">
        <!-- BEGIN PAGE HEADER-->
        <!-- BEGIN PAGE BAR -->
        <div class="page-bar {{(Auth::user()->isAllow('wmo_cilfc_add')) ? "" : "hidden"}}">
        ...
        <!-- END PAGE BAR -->
        <!-- BEGIN PAGE TITLE-->
        <h3 class="page-title">References Company Module
        ...
        <div class="m-heading-1 border-green m-bordered col-md-4">
            <h3>Countries:</h3>
            <p>
                <select id="ulke_id" class="form-control select2" onchange="listesiGetir()">
                    <option value="/">Select Country</option>
                    @foreach($ulkeler_liste as $row)
                        <option value="{{$row->id}}"{{$row->id == $ulke_id ? " selected" : ""}}>{{$row->adi." - ".$row->ulke_sayisi}}</option>
                    @endforeach
                </select>
            </p>
                                ...
                                <table class="table table-condensed table-hover">
                                    ...
                                    @foreach($liste as $key => $row)
                                        <tr>
                                            ...
                                            <td>{{$row->adi}}</td>
                                            <td>{{$row->ulke->adi}}</td>
                                            <td>{{$row->sektor->adi}}</td>
                                            <td>@if($row->web_sayfasi != "")<a href="{{$row->web_sayfasi}}" target="_blank">Click to website</a>@endif</td>
                                        </tr>
                                    @endforeach
                                    </tbody>
                                </table>
